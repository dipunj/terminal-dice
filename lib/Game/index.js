...
class GameOfDice {
	...
	async beginGame() {
		...
		while (this.gameboard.standings.length !== this.gameboard.playerCount) {
			...
			const { length } = player.rollHistory;
			let flag = false;
			if (length >= 3) {
				const {
					[length - 1]: last,
			...
			// on a six, player gets to play again or if the sum of last three turns is 10 and if his game is not finished yet
			if (diceValue !== 6 && !flag) {
				this._incrementTurn();
			}
		}
	...
	getCurrentScoreBoard() {
		return [
			["ID", "Name", "Last Three Turns", "Score"],
